...
import { reduceStream } from '../../index.js'

const sleep = promisify(setTimeout)
const sockPath = '/tmp/var/run/numaflow/reduce-stream.sock'
const infoPath = '/tmp/var/run/numaflow/reduce-stream-info.sock'
...
test('reduce stream integration test', async () => {
    ...
    try {
        ...
        const cargoProcess = spawn('cargo', ['run', '-p', 'tests', '--bin', 'reducestream', '--', sockPath], {
            stdio: 'pipe',
        })
