...
class StreamSorter {
    ...
    async *streamSorter(datums: accumulator.DatumIterator) : AsyncIterable<accumulator.Message> {
        ...
        for await (let datum of datums) {
            datum_count += 1;
            console.log(`Received datum ${datum_count}: event_time=${datum.eventTime}, watermark=${datum.watermark}, value=${datum.value}`);
        ...
        if (this.sorted_buffer.length > 0) {
            ...
            for (let datum of this.sorted_buffer) {
                console.log(`Flushing datum: event_time=${datum.eventTime}, watermark=${datum.watermark}, value=${datum.value}`);
                yield {
                    keys: datum.keys,
                    value: datum.value,
                    id: datum.id,
